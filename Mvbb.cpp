...
int nearHull_2D(const vector< Point2d >& P, unsigned int k, vector< Point2d > &H, bool convex=true )
{
...
MVBB::MVBB(const vector<double>& points, const char* bbivfile, bool redux) 
{  
...
MVBB::MVBB(const vector<Point>& points, const char* bbivfile, bool redux) 
{  
  //
  _numPoints = points.size();
...
void MVBB::divide(const vector<double>& points, double X[3], double Y[3], double Z[3],
...
void MVBB::argVector2point3dVector(const vector<double>& points, vector<Point3d> &points3d) 
{	
#ifdef _DEBUG_
...
void MVBB::point3dVector2DoubleVector(const vector<Point3d>& points3d, vector<double> &points) 
{					     
#ifdef _DEBUG_
...
void MVBB::calMaxMin(const vector<Point3d> &points3d,
...
int MVBB::findBestHull(const vector<Point2d>& points, Point2d &opt_p, Point2d &opt_q, float &minV) 
{
  unsigned int RES = 4;
  unsigned int i,j,k;
  vector<Point2d> hull;
                       ...
                       int &opt_face, Point2d &opt_p, Point2d &opt_q) 
{
  ...
  //numberOfCHcuts[0] = 
  findBestHull(points_xy, p, q, vol);
  ...
  //numberOfCHcuts[1] = 
  findBestHull(points_xz, p, q, vol);
  ...
  //numberOfCHcuts[2] = 
  findBestHull(points_yz, p, q, vol);
...
void MVBB::makeCut(const vector<Point3d>& in, int cut_idx, Point2d cut_p, Point2d cut_q,
...
double MVBB::volume()
{
    return ((bbCSet.begin())->bb).volume();
}

std::vector<float> MVBB::bbox_side_lengths()
{
    ((bbCSet.begin())->bb).volume();

    std::vector<float> sides;
    sides.push_back(((bbCSet.begin())->bb).high_1 - ((bbCSet.begin())->bb).low_1);
    sides.push_back(((bbCSet.begin())->bb).high_2 - ((bbCSet.begin())->bb).low_2);
    sides.push_back(((bbCSet.begin())->bb).high_3 - ((bbCSet.begin())->bb).low_3);
    return sides;
}

std::vector<float> MVBB::bbox_center()
{
    ((bbCSet.begin())->bb).volume();

    std::vector<float> center;
    center.push_back((((bbCSet.begin())->bb).high_1 + ((bbCSet.begin())->bb).low_1)/2);
    center.push_back((((bbCSet.begin())->bb).high_2 + ((bbCSet.begin())->bb).low_2)/2);
    center.push_back((((bbCSet.begin())->bb).high_3 + ((bbCSet.begin())->bb).low_3)/2);
    return center;
}

std::vector<float> MVBB::bbox_side_directions()
{
    ((bbCSet.begin())->bb).volume();

    std::vector<float> directions;
    directions.push_back(((bbCSet.begin())->bb).dir_1[0]);
    directions.push_back(((bbCSet.begin())->bb).dir_1[1]);
    directions.push_back(((bbCSet.begin())->bb).dir_1[2]);

    directions.push_back(((bbCSet.begin())->bb).dir_2[0]);
    directions.push_back(((bbCSet.begin())->bb).dir_2[1]);
    directions.push_back(((bbCSet.begin())->bb).dir_2[2]);

    directions.push_back(((bbCSet.begin())->bb).dir_3[0]);
    directions.push_back(((bbCSet.begin())->bb).dir_3[1]);
