...
{	
  ...
  MvbbParameters() : Gain(0.99), MinPoints(4), Samples(100), Grid(3), MinCutCells(50) {};
...
class MVBB
{
 public:
   ...
   */
  MVBB(const vector<double>& coords, const char* bbivfile=NULL, bool redux=false);
   ...
   * @param redux  If points shall be reduced in the decomposition stage.
   */
  MVBB(const vector<Point>& points, const char* bbivfile=NULL, bool redux=false);
  ...
  /**
   * Gives the volume of the bounding box having id 'box_id'
  */
  double volume();

  /**
   * Gives the dimensions of the bounding box having id 'box_id'
  */
  std::vector<float> bbox_side_lengths();


  /**
   * Gives the center of the bounding box
  */
  std::vector<float> bbox_center();

  /**
   * Gives the dimensions of the bounding box having id 'box_id'
  */
  std::vector<float> bbox_side_directions();
 ...
 private:
   ...
   */
  void divide(const vector<double>& points, double X[3], double Y[3], double Z[3],
   ...
   */
  void argVector2point3dVector(const vector<double>& points, vector<Point3d> &points3d);
   ...
   */
  void point3dVector2DoubleVector(const vector<Point3d>& points3d, vector<double> &points);
   ...
   */
  void calMaxMin(const vector<Point3d>& points3d,
   ...
   */
  int findBestHull(const vector<Point2d>& points, Point2d &opt_p, Point2d &opt_q, float &minV);
   ...
   */
  void makeCut(const vector<Point3d>& in, int cut_idx, Point2d cut_p, Point2d cut_q,
